...
public class ControllerFirstAdmin {
	...
	*/
	 ...
	 * account.  We will first validate if the username and the password of the new admin account can 
	 * pass our test of scrutiny to ensure they are compliant with our format. 
	 * It start by trying to establish a new user and placing that user into the
	...
	protected static void doSetupAdmin(Stage ps, int r) {
		...
		// Check to see our username has passed validation 
		String usernameValidation = UserNameRecognizer.checkForValidUserName(adminUsername);
		if (usernameValidation.length() > 0) {
			ViewFirstAdmin.label_PasswordsDoNotMatch.setText(usernameValidation);
			return;
		}
