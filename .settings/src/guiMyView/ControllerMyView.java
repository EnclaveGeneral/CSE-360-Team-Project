...
public class ControllerMyView {
	...
	**********************************************************************************************/
	...
	protected static void selectPost() {
		...
		if (p.isImagePost()) {
			...
			ViewMyView.radio_Image.setSelected(true);
			ViewMyView.label_ImageFile.setText(p.getImageFilename() != null
				? p.getImageFilename() : "image post");
			ViewMyView.text_Body.setText("");
		} else {
			// Switch toggle to TEXT and populate the body field
			ViewMyView.radio_Text.setSelected(true);
			ViewMyView.text_Body.setText(p.getBody() != null ? p.getBody() : "");
		}
	...
	**********************************************************************************************/
	...
	protected static void performCreatePost() {
		String author = ViewMyView.text_Author.getText().trim();
		String title  = ViewMyView.text_Title.getText().trim();
		String tags = ViewMyView.text_tags.getText().trim();

		String authorErr = ModelMyView.validateAuthor(author);
		if (!authorErr.isEmpty()) { setError(authorErr); return; }

		String titleErr = ModelMyView.validateTitle(title);
		if (!titleErr.isEmpty()) { setError(titleErr); return; }

		if (ViewMyView.radio_Image.isSelected()) {
			// Image post — validate file and write bytes to the database
			String fileErr = ModelMyView.validateImageFile(pendingImageFile);
			...
			String body    = ViewMyView.text_Body.getText().trim();
			String bodyErr = ModelMyView.validateBody(body);
	...
	**********************************************************************************************/
	...
	protected static void performCreateReply() {
		...
		String author  = ViewMyView.text_Author.getText().trim();
		String body    = ViewMyView.text_ReplyBody.getText().trim();
		...
		ViewMyView.text_ReplyBody.setText("");
	...
	**********************************************************************************************/
	...
	private static void clearPostFields() {
		ViewMyView.text_Author.setText("");
		ViewMyView.text_Title.setText("");
		ViewMyView.text_Body.setText("");
		ViewMyView.radio_Text.setSelected(true);
		ViewMyView.label_ImageFile.setText("No file selected.");
		pendingImageFile = null;
